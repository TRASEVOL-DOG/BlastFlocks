...
function _init()
 ...
 while t<introt do
  ...
  for i=0,3999 do
   ...
   if c==7 and kt>=1 then
    --we're good
   elseif c>=16-kt*2 then
    if rnd(2)<1 then
     c=drk[c]
    end
   else
...
function init_game()
 ...
 for i=1,256 do
  --create_ship(rnd(128),400+rnd(128),(rnd(8)<1) and "biggie" or "smol",true)
 end
 
 for i=1,512 do
  --create_ship(1024+rnd(128),1024+rnd(128),(rnd(8)<1) and "biggie" or "smol")
 end
 
-- create_helixship(600,600)
-- create_hole(400,400,12)
...
function draw_mainmenu()
 ...
 if curmenu=="mainmenu" then
  font("pico16")
  --draw_text("[[TITLE]]",scrnw/2,scrnh*0.2)
...
--[[  do
   all_colors_to(0)
   foo(0,-2) foo(-1,-1) foo(1,-1)
   foo(-2,0) foo(2,0)
   foo(-2,1) foo(2,1)
   foo(0,3) foo(-1,2) foo(1,2)
   all_colors_to(13)
   foo(-1,1) foo(0,2) foo(1,1)
   all_colors_to(7)
...
-- local spd=1
-- if btn(0) then s.x=s.x-spd end
-- if btn(1) then s.x=s.x+spd end
-- if btn(2) then s.y=s.y-spd end
-- if btn(3) then s.y=s.y+spd end
...
--    draw_to()
--    local tinsurf=new_surface()
--    draw_to(tinsurf)
--    draw_surface(surf,0,0,s.ox-s.w/2,s.oy-s.h/2,s.w,s.h)
--    draw_to(surf)
--    rectfill(s.ox-s.w/2,s.oy-s.h/2,s.ox+s.w/2,s.oy+s.h/2,0)
--    
--    local oplt={2,14,15,7,6,13}
--    local plt={}
--    for i=1,5 do
--     plt[oplt[i]]=8+(s.c+i)%8
--    end
--    apply_pal_map(plt)
--    draw_surface(tinsurf,s.x-s.w/2,s.y-s.h/2)
--    all_colors_to()
...
-- plt={2,14,15,15,14,7,6}
-- plt={2,8,15,15,14,7,6}
-- plt={0,1,7,6,12,7,6}
-- plt={4,9,15,15,6,7,10}
-- plt={4,15,7,15,9,7,6}
 local c=plt[1]
